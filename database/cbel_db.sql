...
DROP EVENT `SomeEvent`; CREATE DEFINER=`root`@`localhost` EVENT `SomeEvent` ON SCHEDULE EVERY 1 DAY STARTS '2014-03-30 00:00:00' ENDS '2038-01-01 00:00:00' ON COMPLETION NOT PRESERVE ENABLE DO UPDATE `cbel_lead` SET `activity_count`=0 WHERE TIMESTAMPDIFF(DAY, CURRENT_TIMESTAMP, `timestamp`) <= -7